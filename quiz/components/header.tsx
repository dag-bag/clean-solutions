...
const Header: React.FC<props> = ({ category, title }) => {
    ...
    return (
        <div className=' flex flex-col justify-center xl:pr-20 md:px-5 xl:pl-0 px-10 py-10'>
            ...
            <p className="text-gray-700 text-lg mb-1">{isToggled ? discription.concat(discription_more) : discription}</p>
            <button onClick={onClickHandler} className="items-center max-w-[150px] justify-start py-2 overflow-hidden  text-green-1 transition-all duration-150 ease-in-out rounded-full bg-none group border border-green-1 my-2 font-normal ">Read {!isToggled ? 'Less' : 'More'}</button>
        </div>
    )
}
